...
inline char read()
{
	...
	s == t ? t = (s = buf) + fread(buf, 1, IN_LEN,
								   stdin) : 0;
	return s == t ? -1 : *s++;
}
...
inline void read(T& x)
{
	...
	for (c = read(), iosig = false; !isdigit(c);
		 c = read())
	{
		if (c == -1) return;
...
struct block
{
	...
	inline long long get_sum(int ql, int qr)
	{
		...
		if (qwq.first == l &&
			qwq.second == r) return this_sum;

		long long summ = 0;

		for (int i = qwq.first; i <= qwq.second;
			 ++i) summ += vals[i].val;
	...
	inline void modify_range(int ql, int qr, int k)
	{
		...
		pair<int, int> t = get_range(ql, qr);

		if (t.first == l && t.second == r) t_sum += k;

		else for (int i = t.first; i <= t.second;
					  ++i) vals[i].val += k;
...
inline void init()
{
	...
	block_num = n % block_size == 0 ? n / block_size :
				n / block_size + 1;
