...
Node t[maxn << 5];
int root[maxn * 10], cnt;
int n, m, x, y, sz, lastans, u, v, k;
int a[maxn], b[maxn], dep[maxn], f[maxn][30];
...
{
    vis[k] = 1, dep[k] = dep[f[k][0]] + 1;
    for (int i = 0; i < edges[k].size(); ++i)
        if(edges[k][i] != f[k][0] && !vis[edges[k][i]]) dfs1(edges[k][i]);
}

inline void dfs2(int k)
{
    vis[k] = 1, root[k] = update(root[f[k][0]], 1, sz, lower_bound(b + 1, b + 1 + sz, a[k]) - b);
    for (int i = 0; i < edges[k].size(); ++i) 
        if(edges[k][i] != f[k][0] && !vis[edges[k][i]]) dfs2(edges[k][i]);
}
...
inline void solve()
{
    ...
    for (int i = 1; i < n; ++i)
        read(x), read(y), addedge(x, y), f[y][0] = x;

    dfs1(1), vis.reset(), dfs2(1);
    for (int i = 0; i <= n; ++i)
