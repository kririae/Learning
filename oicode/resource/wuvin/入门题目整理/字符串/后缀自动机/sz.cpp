...
int cnt,root,last;int w,nw,k,lca;
inline void add(const int ch,const int fr){
	w=last;last=nw=++cnt;len[nw]=len[w]+1;chu[nw]=fr;
	while(w&&ne[w][ch]==0) ne[w][ch]=nw,w=par[w];
	if(w){
		k=ne[w][ch];
		if(len[k]!=len[w]+1){
			lca=++cnt;memcpy(ne[lca],ne[k],sizeof(ne[k]));par[lca]=par[k];chu[lca]=chu[k];
			len[lca]=len[w]+1;par[k]=par[nw]=lca;
