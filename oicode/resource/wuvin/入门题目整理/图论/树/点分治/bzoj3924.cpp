...
void addedge(int f,int t,int dis){
    e[++ecnt]=(edge){t,head[f],dis};head[f]=ecnt;
    e[++ecnt]=(edge){f,head[t],dis};head[t]=ecnt;
}
...
void LCA_DFS(int u,int fr,int now_dis){
    ETTx[++ETcnt]=u;dis[u]=now_dis;show_pos[u]=ETcnt;size[u]=1;fa[u]=fr;
    iter(i,u){
        if(e[i].t!=fr){
            LCA_DFS(e[i].t,u,now_dis+e[i].dis);
            ETTx[++ETcnt]=u;size[u]+=size[e[i].t];
        }
    }
}
...
vector<LL> dsubtree_wei[N];LL dsum_wei[N];//重量和
vector<LL> dsubtree_ans[N];LL dsum_ans[N];//答案贡献
vector<int> belong[N];//属于哪颗子树

int buf_size[N];
void mark_DFS(int u,int bel,int fr){
    buf_size[u]=1;belong[u].push_back(bel);
    iter(i,u){
...
void DFS2(int u,int fr,int tp){
    top[u]=tp;DFSx[++dcnt]=u;ch_pos[u]=dcnt;
    if(size[u]==1) return;
...
void pushdown(int k){
    if(a[k].son[L]!=0&&a[k].add_tag!=0){
        xds &l=a[a[k].son[L]],&r=a[a[k].son[R]];int &t=a[k].add_tag;
        l.mx+=t,r.mx+=t,l.add_tag+=t,r.add_tag+=t;t=0;
    }
}
void update(int k){
...
void add(int k,int l,int r,int val){
    pushdown(k);
    if(a[k].l==l&&a[k].r==r){
        a[k].mx+=val;a[k].add_tag+=val;return;
    }
