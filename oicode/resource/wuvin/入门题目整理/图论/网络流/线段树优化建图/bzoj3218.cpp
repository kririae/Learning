...
//-------------------------------------------------------------
const int INF=99999999;int S,T,sz;
...
bool BFS(){//分层 
	...
	while(!x.empty()){
		...
		for(int i=0,len=node[u].size();i<len;i++){
			...
			dis[ed.to]=dis[u]+1;x.push(ed.to);
		}
	}
    return dis[T]!=0;
}
int DFS(int u,int a){//多路增广  a-可行流 
	if(u==T||a==0) return a;int flow=0;
	for(int &i=cur[u],len=node[u].size();i<len;i++){//pay attention to &i 
		...
		if(f){a-=f;flow+=f;ed.flow-=f;e[node[u][i]^1].flow+=f;if(a==0) return flow;}
	}
	return flow;
}
ints Dinic(){
	ints maxflow=0;
	while(BFS()){memset(cur,0,sizeof(int)*(sz+2));maxflow+=DFS(S,INF);}
	return maxflow;
}
//-----------------------------------------------------------------
int n,cnt,valsz;ints sumb,sumw;
int *lsh[5005*3];int lscnt;
struct block{
    int a,b,w,l,r,p;
}bl[5005];
//-----------------------------------------------------------------
...
void init(){
    n=read();lscnt=0;
    for(int i=1;i<=n;i++){
        bl[i].a=read();bl[i].b=read();bl[i].w=read();bl[i].l=read();bl[i].r=read();bl[i].p=read();
        lsh[++lscnt]=&bl[i].a;lsh[++lscnt]=&bl[i].l;lsh[++lscnt]=&bl[i].r;
        sumb+=bl[i].b;sumw+=bl[i].w;
    }
}
...
void insert(int &k,int fr,int to,int v,int cnn){
    int t=++xdscnt;xds[t].son[L]=xds[k].son[L];xds[t].son[R]=xds[k].son[R];xds[t].name=++cnt;
    if(k) addedge(xds[t].name,xds[k].name,INF);k=t;
    if(fr==to){
