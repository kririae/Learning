...
//费用流
struct edge{
	int from,to,flow,cap;double cost;
	edge(int f,int t,int fl,double co,int capture){
		from=f;to=t;flow=fl;cap=capture;cost=co;
	}
};
vector<edge> e;
int len=0;
...
long double feiyong(int start,int end,int &flow){
	...
	distan[301]=0;distan[1]=1;
	memset(a,0,sizeof(a));
	//spfa
	queue<int> x;
	x.push(start);distan[start]=1;inq[start]=true;
	a[start]=flow;
	while(!x.empty()){
	...
	int p=end;
	while(p!=start){
		e[from[p]].flow+=a[end];e[from[p]^1].flow-=a[end];p=e[from[p]].from;
	}
	flow-=a[end];
