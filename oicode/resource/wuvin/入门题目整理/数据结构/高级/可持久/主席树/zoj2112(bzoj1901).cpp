...
inline void addmark(const int &k,const int &c){
    if(k==0)return;tag[k]+=c;
}
inline void pushdown(const int &k){
    if(k==0||tag[k]==0) return;data[k]+=tag[k];
    addmark(son[k][L],tag[k]);addmark(son[k][R],tag[k]);tag[k]=0;
}
inline void rotate(int &k,int s){
    int t=son[k][s];son[k][s]=son[t][s^1];son[t][s^1]=k;k=t;
}
void add(int &k,const int &c,const int &vis,int &minv){//minv-请出示化为0
    if(k==0){
        k=++cnt;son[k][L]=son[k][R]=0;pri[k]=rand();data[k]=c+minv;vision[k]=vis;tag[k]=0;
    }else{
        pushdown(k);
...
inline void bitadd(int x,int vis,int f){//f==1
    for(int i=x,z=0;i<N+N;i+=low(i)){
        z=0;add(rootbit[i],f,vis,z);
    }
}
inline int kth(int start,int end,int k){//区间[start,end]的k大
    int at=0;start--;
    for(int i=14;i>=0;i--){
        at+=1<<i;if(at>N+N) {at-=1<<i;continue;}
        int t1=0,t2=0;get(rootbit[at],start,t1);get(rootbit[at],end,t2);
        if(t2-t1>=k) at-=1<<i;
        else k-=t2-t1;
    }
    return at+1;
...
void init(){
    ...
    for(int i=1;i<=m;i++){
        scanf("%s",c);
        if(c[0]=='Q'){
            scanf("%d%d%d",&a1[i],&a2[i],&a3[i]);opera[i]=1;
        }else{
            scanf("%d%d",&a1[i],&a2[i]);opera[i]=2;
            p[++pcnt].a=&a2[i];
        }
    }
}
void pre(){
    sort(p+1,p+pcnt+1);p[1].name=1;
