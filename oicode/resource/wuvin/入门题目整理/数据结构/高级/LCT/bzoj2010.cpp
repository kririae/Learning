...
struct splay_tree{
	...
	inline void pushdown(const int &k){
    	if(k==0) return;node& t=a[k];
    	if(t.size==1){t.turn=0;return;}
    	if(t.turn){
			swap(t.son[L],t.son[R]);t.turn=0;
			a[t.son[L]].turn^=1;a[t.son[R]].turn^=1; 
		}
	}
	...
	inline int splay_root(int k){
		int t=get_root(k);int t2=kth(t,1);splay(t2,t);
		return t2; 
	}
	...
	inline int insert(int k,int place,int val_){//在k的子树中作为第place小  请确保k是root 
		int last=a[k].fa;int end=k;
		while(1){
			if(k==0){
				k=++cnt;a[k].fa=last;a[k].val=val_;a[k].size=1;break;
			}else{
				a[k].size++;
				int t=a[a[k].son[L]].size+1;
				if(place<=t){
					last=k;k=a[k].son[L];continue;
				}else{
					last=k;k=a[k].son[R];place-=t;continue;
				}
			}
		}
		splay(k,end);
...
struct LCT{
	...
	inline int newtree(int val_){
		int k=++hh.cnt;hh.a[k].fa=0;hh.a[k].size=1;
		hh.a[k].son[L]=0;hh.a[k].son[R]=0;hh.a[k].val=val_;
	}
	inline void make(int k,int fath,int value){
		b[k].fa=fath;b[k].val=value;
	}
