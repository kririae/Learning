...
inline void make_same(int k, int c)
{
	a[k].sum = a[k].size * c;
	a[k].c_tag = c;
	a[k].lseq = a[k].rseq = a[k].mseq = max(c, a[k].sum);
	a[k].val = c;
}
inline void pushdown(int k)
{
	if(k == 0) return;
	if(a[k].son[L] + a[k].son[R] == 0) {
		a[k].t_tag = 0;
		a[k].c_tag = -INF;
		return;
	}
...
inline void update(int k)
{
	...
	a[k].size = 1;
	a[k].sum = a[k].val;
	a[k].lseq = a[k].rseq = a[k].mseq = a[k].val;
	if(a[k].son[L] != 0) {
		node& l = a[a[k].son[L]];
...
void build(int &k, int l, int r, int fr, const int* buf)
{
	...
	int mid = (l + r) >> 1;
	a[k].val = buf[mid];
	a[k].son[L] = a[k].son[R] = 0;
	if(l == r) {}
	else if(l + 1 == r) {
		build(a[k].son[R], r, r, k, buf);
	} else {
		build(a[k].son[L], l, mid - 1, k, buf);
		build(a[k].son[R], mid + 1, r, k, buf);
	}
	update(k);
	return;
}
