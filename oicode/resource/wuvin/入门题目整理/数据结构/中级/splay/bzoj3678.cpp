...
int make(int k,int x){//分裂 
	...
	if(x==1){
		int t=++cnt;a[t].fa=k;a[t].height=a[k].height-1;
		a[t].son[R]=a[k].son[R];a[t].val=a[k].val+1;a[a[k].son[R]].fa=t;
		update(t);a[k].height=1;a[k].son[R]=t;return k;
	}
	int t=++cnt;a[t].fa=k;a[t].height=a[k].height-x+1;a[t].size=a[t].height;
	a[t].son[R]=a[k].son[R];a[t].val=a[k].val+x-1;a[a[k].son[R]].fa=t;
	a[k].height=x-1;a[k].son[R]=t;update(t);make(t,1);
}
inline int kth(int k,int x){//非递归好像更快 
    ...
    while(1){
        ...
        if(t1<x){k=a[k].son[R];x-=t1;continue;}
...
void build(int &k,int l,int r,int from){
	k=++cnt;a[k].fa=from;a[k].size=r-l+1;a[k].son[L]=0;a[k].son[R]=0;a[k].height=1;
	int mid=(l+r)>>1;a[k].val=hard[mid];
	if(l==r) return;
...
int main(){
	...
	for(int i=1;i<=m;i++){
		...
		if(ins==0){
			scanf("%d%d%d",&p,&a2,&b);
			int root2=++cnt;a[root2].height=b-a2+1;a[root2].size=b-a2+1;a[root2].val=a2;
			get(root,p+1,p);a[a[root].son[R]].son[L]=root2;
			a[root2].fa=a[root].son[R];
			update(a[root].son[R]);update(root);
		}else if(ins==1){
			scanf("%d%d",&a2,&b);
			int t=get(root,a2,b);a[t].fa=0;
