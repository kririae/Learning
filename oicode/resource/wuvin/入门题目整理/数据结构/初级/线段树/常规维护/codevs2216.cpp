...
inline void add_one_number(int k,int a){
    if(k>=nn){sum[k]+=a,sum[k]%=P;return;}
    adds[k]+=a;sum[k]+=size[k]*a;sum[k]%=P;adds[k]%=P;
}
inline void update(int k){
    if(k>=nn) return;
    sum[k]=sum[k<<1]+sum[(k<<1)|1];sum[k]*=multi[k];sum[k]+=adds[k]*size[k];sum[k]%=P;
}
inline void multi_one(int k,int a){
    if(k>=nn){sum[k]*=a,sum[k]%=P;return;}
    adds[k]*=a;adds[k]%=P;multi[k]*=a;multi[k]%=P;update(k);
}
...
void add(int l,int r,int c){
	push_down(l+nn-1);push_down(r+nn+1);
...
int main(){
    ...
    for(int i=nn+1;i<=nn+n;i++){
        scanf("%d",&sum[i]);size[i]=1;multi[i]=1;adds[i]=0;
    }
    for(int i=nn-1;i>0;i--){
        sum[i]=sum[i<<1]+sum[(i<<1)|1];size[i]=size[i<<1]+size[(i<<1)|1];multi[i]=1;adds[i]=0;
    }
    scanf("%d",&m);
    for(int i=1;i<=m;i++){
        int ins,t,g,c;
