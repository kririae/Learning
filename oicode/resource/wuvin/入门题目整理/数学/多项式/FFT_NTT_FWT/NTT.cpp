...
int fastpow(int w,int x){
	if(x==1) return w;
	int t=fastpow(w,x>>1);t=(LL)t*t%mod;
...
int main(){
	scanf("%d%d",&n,&m);int len=1;while((1<<len)<n+m+1) len++;
	for(int i=1;i<(1<<len);i++) rv[i]=(rv[i>>1]>>1)|((i&1)<<(len-1));
	wn[N]=1;int w=fastpow(3,(mod-1)>>len);len=1<<len;int rv=fastpow(w,mod-2);
	for(int i=1;i<len;i++) wn[N+i]=((LL)wn[N+i-1]*w)%mod;for(int i=N-1;i>=N-len;i--) wn[i]=((LL)wn[i+1]*rv)%mod;
