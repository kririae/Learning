...
void init() {
	...
	for(int i = 1; i < n; i++) {
		int f, t, d;
		scanf("%d%d%d", &f, &t, &d);
		node[f].x.push_back((edge) {
			f, t, d
		});
		node[t].x.push_back((edge) {
			t, f, d
		});
	}
}
