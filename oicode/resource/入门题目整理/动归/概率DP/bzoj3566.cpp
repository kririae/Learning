...
#define N 500005
using namespace std;
struct edge{int to,next;double p;};
edge e[2*N];int elen=0;
int head[N];
void addedge(int f,int t,double p){
    e[++elen]=(edge){t,head[f],p};head[f]=elen;
    e[++elen]=(edge){f,head[t],p};head[t]=elen;
}
inline double fabs(double k){return k<0?-k:k;}
//---------------------------------------
int n,pi[N];
double f[N];
