...
point buf[N];int cnt;
int name_buf[N];int name_top;
void build(int &k,int fr,int to,int p){
    KD=p;int mid=(fr+to)>>1;
    nth_element(buf+fr,buf+mid,buf+to+1);
    k=name_buf[name_top--];
    kd[k].me=buf[mid];kd[k].son[0]=kd[k].son[1]=0;
...
void insert(int &k,const point &p,int hua){
    if(k==0) {k=++kd_cnt;kd[k].me=p;kd[k].son[L]=kd[k].son[R]=0;update(k);return;}
    kd[k].size++;
...
void get_near(int k,const point &p){
    ...
    LIM=min(LIM,t);
	int d[2];d[L]=judge(kd[k].son[L],p);d[R]=judge(kd[k].son[R],p);
    int s=d[L]<d[R]? L:R;
