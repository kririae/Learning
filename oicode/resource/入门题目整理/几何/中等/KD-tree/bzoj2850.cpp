...
inline void update(int k){
	...
	for(int i=0;i<M;i++){
		a[k].mm[i]=a[k].key[i];a[k].mx[i]=a[k].key[i];
		for(int s=0;s<2;s++){
			if(a[k].son[s]!=0){
...
int build(int fr,int to,int mmx,int mxx,int mmy,int mxy){
    ...
    a[sor[mid]].p=canshu;int k=sor[mid];
    if(canshu==0){
...
inline int judge(int k){
	int bl=check(a[k].mm[0],a[k].mm[1]);int ret=bl;
	bl=check(a[k].mx[0],a[k].mm[1]);if(bl^ret) return -1;
	bl=check(a[k].mm[0],a[k].mx[1]);if(bl^ret) return -1;
	bl=check(a[k].mx[0],a[k].mx[1]);if(bl^ret) return -1;
	return ret;
}
LL get_left(int k){
	if(k==0) return 0;
...
int main(){
    scanf("%d%d",&n,&m);
    int mmx,mxx,mmy,mxy;
	mmx=mmy=INF;mxx=mxy=-INF; 
    for(int i=1;i<=n;i++){
    	int aa,bb,cc;
    	scanf("%d%d%d",&aa,&bb,&cc);
    	a[i].key[0]=aa;a[i].key[1]=bb;a[i].val=cc;
    	mmx=min(mmx,aa);mxx=max(mxx,aa);
    	mmy=min(mmy,bb);mxy=max(mxy,bb);
	}
	for(int i=1;i<=n;i++) sor[i]=i;
	int root=build(1,n,mmx,mxx,mmy,mxy);a[0].p=-1;
	for(int i=1;i<=m;i++){
		scanf("%d%d%d",&A,&B,&C);
		printf("%lld\n",get_left(root));
	}
