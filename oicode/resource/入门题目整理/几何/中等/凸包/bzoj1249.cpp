...
int main()
{
	...
	xx = (x1 + x2 + x3) / 3.0;
	yy = (y1 + y2 + y3) / 3.0;
	point p1 = (point) {x1, y1}, p2 = (point) {x2, y2}, p3 = (point) {
		x3, y3
	};
	ans = abs(cross(p2 - p1, p3 - p1));
	if(ans != 0) {
		st.insert((node) {
			atan2(p1.y - yy, p1.x - xx), p1
		});
		st.insert((node) {
			atan2(p2.y - yy, p2.x - xx), p2
		});
		st.insert((node) {
			atan2(p3.y - yy, p3.x - xx), p3
		});
	} else { //三点共线
	...
	for(int i = 1; i <= n; i++) {
		...
		if(ans == 0) {
			...
			if(ans != 0) {
				...
				st.insert((node) {
					atan2(base.y - yy, base.x - xx), base
				});
				st.insert((node) {
					atan2(p1.y - yy, p1.x - xx), p1
				});
				st.insert((node) {
					atan2(buf.y - yy, buf.x - xx), buf
				});
			} else {
