...
#define N 20005
using namespace std;
int par[N*2],len[N*2],rl[N*2],rr[N*2];short ne[N*2][180];short yw[N*2][180];int sz[N*2];//来压位一发挤个rank 
...
//204ms 
int cnt,root,last;
int w,nw,lca,k;int buf; 
inline void add(int ch,int val){
	w=last;last=nw=++cnt;len[nw]=len[w]+1;rl[nw]=rr[nw]=val;
	while(w&&ne[w][ch]==0) ne[w][ch]=nw,yw[w][++sz[w]]=ch,w=par[w];
	if(w){
		k=ne[w][ch];
		if(len[k]!=len[w]+1){
			lca=++cnt;//memcpy(ne[lca],ne[k],sizeof(ne[k]));
			for(int i=1;i<=sz[k];i++) ne[lca][yw[k][i]]=ne[k][yw[k][i]],yw[lca][i]=yw[k][i];sz[lca]=sz[k]; 
			par[lca]=par[k];len[lca]=len[w]+1;rr[lca]=rr[k];rl[lca]=rl[k];
			par[k]=par[nw]=lca;
...
int n;
inline int read(){
	static char c;static int ret;c=getchar();ret=0;
