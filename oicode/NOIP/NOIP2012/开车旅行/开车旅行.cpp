...
inline void init()
{
	...
	for (int i = n; i >= 1; --i) {
		...
		if(ptr != st.begin()) {
			next = (--ptr) -> first, ++ptr, next_index = (--ptr) -> second, ++ptr;
		}
		if(ptr != st.end()) {
			pre = (++ptr) -> first, --ptr, pre_index = (++ptr) -> second, --ptr;
		}
		// 上面的aa next pre dd分别算出了该数周围四个数字的大小...但是可能有bug...一会儿再来调...
		// 分别为前一个 前两个 后一个 后二个...
		// 下一步是初始化最短的
		if(abs(h[i] - next) <= abs(pre - h[i])) 
			next[i].p = abs(h[i] - next), next[i].pidx = next_index, next[i].pp = abs(pre - h[i]), next[i].ppidx = pre_index;
		else next[i].p = abs(pre - h[i]), next[i].pidx = pre_index, next[i].pp = abs(h[i] - next), next[i].ppidx = next_index;
	}
	for (int i = 1; i <= n; ++i) {
