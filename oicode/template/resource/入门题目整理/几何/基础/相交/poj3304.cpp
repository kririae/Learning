...
int main(){
	...
	while(T--)
	{
        ...
        for(int i=1;i<=n;i++){
        	if(flag) break;
			for(int j=1;j<=n;j++){
           	   if(jud(s[i].a,s[j].a)){flag=1;break;}
           	   if(jud(s[i].a,s[j].b)){flag=1;break;}
           	   if(jud(s[i].b,s[j].a)){flag=1;break;}
           	   if(jud(s[i].b,s[j].b)){flag=1;break;}
           }
		}
