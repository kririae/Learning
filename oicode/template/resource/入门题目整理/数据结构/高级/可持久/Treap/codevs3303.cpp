...
void pushdown(int k){
    if(a[k].rv==0) return;
    a[k].rv=0;swap(a[k].son[L],a[k].son[R]);
    a[a[k].son[L]].rv^=1;a[a[k].son[R]].rv^=1;
}
...
void insert(int &root,int val){
    int t=++cnt;a[t].pri=rand()*rand()%INF;a[t].val=val;a[t].size=1;
    root=merge(root,t);
}
void getseg(int root,int l,int r,int &tl,int &tm,int &tr){
    split(root,r,tr);split(root,l-1,tm);tl=root;
}
int root;int n,m;
