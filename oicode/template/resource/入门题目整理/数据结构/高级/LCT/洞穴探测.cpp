...
inline int ch_top(int x){x=splay_rt(x);while(a[x].son[L^a[x].rv]) x=a[x].son[L^a[x].rv];splay(x,splay_rt(x));return x;}//链端 
inline void chg_right(int x,int b){splay(x,splay_rt(x));pushdown(x);a[x].son[R]=b;}
...
inline void make_rt(int x){access(x);splay(x,splay_rt(x));a[x].rv^=1;}
