...
int kth(int k,int fr,int to,int x){
	if(fr==to) return fr;
	int mid=(fr+to)/2;int t=xds[xds[k].son[R]].size;
...
inline int ask(int x){
	int t=ff(qu[x].v);int t2=qu[x].k;
	if(xds[root[t]].size<t2){return -1;}
	else {return kth(root[t],-INF,INF,t2);}
}
inline void addedge(int k){
	int f=ff(e[k].f),t=ff(e[k].t);
	if(f==t) return;
	connect(f,t);root[t]=merge(root[t],root[f]);
}
//--------------------------
int sor[N*5];
...
int main(){
	...
	for(int i=1,t;i<=n;i++){//可以离散化优化 空间 
		scanf("%d",&t);build(root[i],-INF,INF,t);
	}
	for(int i=1;i<=m;i++){
