...
inline void pushdown(const int &k){
    if(k==0) return;node& t=a[k];
    if(t.size==1){
        t.add=0;t.turn=0;return;
    }
    if(k!=0&&t.turn==1){
        swap(t.son[L],t.son[R]);t.turn=0;
        ls.turn^=1;rs.turn^=1;
    }
    if(k!=0&&t.add!=0){
    	if(t.son[L]){ls.add+=t.add;ls.maxn+=t.add;ls.val+=t.add;}
    	if(t.son[R]){rs.add+=t.add;rs.maxn+=t.add;rs.val+=t.add;}
        t.add=0;
    }
}
...
inline int kth(int x){//非递归好像更快 
    int k=root;
    while(1){
        pushdown(k);int t=a[a[k].son[L]].size+1;
        if(t==x) return k;
        if(t<x){k=a[k].son[R];x-=t;continue;}
...
inline void turn_around(const int &l,const int &r){
    int t=get(l,r);a[t].turn^=1;
}
inline int getmax(const int &l, const int &r){
    int t=get(l,r);return a[t].maxn;
}
inline void adds(const int &l,const int &r,const int &v){
    node &t=a[get(l,r)];t.add+=v;t.val+=v;t.maxn+=v;
}
int main(){
    read(n);read(m);
    for(int i=1;i<=n+2;i++){
    	a[i].son[L]=i-1;a[i].fa=i+1;a[i].maxn=0;a[i].val=0;a[i].size=i;
    }
    root=n+2;a[0].maxn=-INF;a[0].val=-INF;a[root].fa=0;
    for(int i=1,k,l,r,v;i<=m;i++){
        read(k);
        if(k==1){
            read(l);read(r);read(v);adds(l,r,v);
        }else if(k==2){
            read(l);read(r);turn_around(l,r);
        }else{
