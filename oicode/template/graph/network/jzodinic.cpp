...
bool bfs(int u, int st)
{
	...
	while(!q.empty()) {
		...
		for(int j = head[now].size() - 1; j >= 0; j--) {
			...
			if(!mp[order].d/*如果连的边为0（没有容量），跳过*/ || layer[end]/*如果已经被标层，跳过*/)
				continue;
...
int dfs(int u, int mn, int t)
{
	...
	for(int i = head[u].size() - 1; i >= 0; i--) {
		...
		if(!now/*流量为0，跳过*/ || layer[end] - 1 != layer[u]/*不是下一层的话，跳过*/) continue;

		int tmp = dfs(end, min(now, mn - ret/*记录流量的最小值*/), t);
...
int main()
{
	...
	for(int u, v, w, i = 1; i <= n; i++) {
		scanf("%d%d%d", &u, &v, &w);
		mp[cnt].num = v, mp[cnt].d = w; //将边标号，终点为v，流量为容量，放入数组中
		head[u].push_back(cnt++);//u连编号为cnt的边 即表示u与v相连，流量为w 该边标号为cnt
