title: 「BZOJ1093」最大半联通子图
author: kririae
date: 2018-11-01 01:49:55
tags:
---
# 「BZOJ1093」最大半联通子图
...
考虑$tarjan$，半联通指对$i \rightarrow j$有边即可。所以对于两个$SCC$之间，有边即可。最大半联通子图，其实就是$SCC$缩点出$DAG$上点数最多的链的个数。
对于这题，该总结的其实是实现细节。
首先第一个，点数最多，把$u \rightarrow v$的边权放到$v$上，$dis[u] = siz[u]$。第二点，实现方面。两个$SCC$中可能有多条边，我们需要去重，对于$tarjan$上计数问题都该如此。
...
inline void tarjan(int x) {
  ...
  if (dfn[x] == low[x]) {
    ++c; 
    do vis[u = s[top--]] = 0, bel[u] = c, ++siz[c]; 
    while (u != x);
  }
}
namespace SP {
