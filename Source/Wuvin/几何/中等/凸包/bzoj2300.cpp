...
inline void insert(point p){
	node t=(node){atan2(p.y-base.y,p.x-base.x),p};
...
void init(){
	...
	m=read();
	for(int i=1;i<=m;i++){
		ci[i].p.x=read();ci[i].p.y=read();
	}
	ci[m+1].p=(point){x,y};
	ci[m+2].p=(point){n,0};
	ci[m+3].p=(point){0,0};
	base=(point){0,0};
	st.insert((node){atan2(0,n),ci[m+2].p});
	st.insert((node){2,ci[m+3].p});
	st.insert((node){atan2(y,x),ci[m+1].p});
