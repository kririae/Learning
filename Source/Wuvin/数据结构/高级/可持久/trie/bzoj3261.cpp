...
void insert(int &k,int x,int p){//p=31..0
	int t=++cnt;a[t].size=a[k].size+1;a[t].son[L]=a[k].son[L];a[t].son[R]=a[k].son[R];k=t;
	if(p==0) return;
...
void ask(int k1,int k2,int x,int p){
	...
	if(a[a[k2].son[s]].size-a[a[k1].son[s]].size!=0){
		ans^=(1<<(p-1));ask(a[k1].son[s],a[k2].son[s],x,p-1);
	}else{
...
int main(){
	scanf("%d%d",&n,&m);
	root[0]=0;insert(root[0],0,31);
	for(int i=1,t;i<=n;i++){
		scanf("%d",&t);root[i]=root[i-1];xall^=t;
	...
	for(int i=1;i<=m;i++){
		...
		scanf("%s",s);
		if(s[0]=='A'){
			scanf("%d",&t);++cnt2;root[cnt2]=root[cnt2-1];xall^=t;
			insert(root[cnt2],xall,31);
		}else{
			scanf("%d%d%d",&l,&r,&t);ans=0;
