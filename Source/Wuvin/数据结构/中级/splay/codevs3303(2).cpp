...
inline void pushdown(int &k){
	...
	swap(son[k][L],son[k][R]);tag[son[k][L]]^=1;tag[son[k][R]]^=1;tag[k]=0;
}
...
inline void rotate(int x,int &k){
	...
	son[y][s]=son[x][s^1];fa[son[x][s^1]]=y;
	son[x][s^1]=y;fa[x]=fa[y];fa[y]=x;
	if(y==k) k=x;
	else{son[z][s2]=x;}
	update(y);update(x);
}
void splay(int x,int &k){
	int y,z;
	while(x!=k){
		y=fa[x];z=fa[y];pushdown(fa[z]);pushdown(z);pushdown(y);pushdown(x);//注意pushdown fa[z]因为是双旋 
		if(y!=k){
...
void turn(int l,int r){
	...
	splay(l,root);splay(r,son[root][R]);
...
int main(){
	scanf("%d%d",&n,&m);
	for(int i=1;i<=n+2;i++){
		meg[i]=i-1;fa[i]=i+1;son[i][L]=i-1;son[i][R]=0;size[i]=i;
	}fa[n+2]=0;root=n+2;
