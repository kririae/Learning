...
void build(char* s,int nlen,int &root){
    int last,p,nw,ch;last=root=cnt=1;len[root]=0;
    for(int i=1;i<=nlen;i++){
        p=last;last=nw=++cnt;len[nw]=len[p]+1;ch=s[i]-'a';
...
char s2[N];int mx[N*2];
void mark(){
    int le=strlen(s2+1);
    int p=rt,p2;int lcnt=0;
    for(int i=1;i<=le;i++){
